...
function CreativeMinds_posted_on() {
	...
	$byline = sprintf(
		esc_html_x( 'Written by %s', 'post author', 'CreativeMinds' ),
	...
	if ( ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
		...
		comments_popup_link( sprintf( wp_kses( __( 'Leave a Comment<span class="screen-reader-text"> on %s</span>', 'CreativeMinds' ), array( 'span' => array( 'class' => array() ) ) ), get_the_title() ) );
		echo '</span>';
	}
...
function CreativeMinds_the_category_list() {
	...
	if ( $categories_list && CreativeMinds_categorized_blog() ) {
		printf( '<span class="cat-links">' . esc_html__( '%1$s', 'CreativeMinds' ) . '</span>', $categories_list ); // WPCS: XSS OK.
	}
}
